...
function buildNamespace(ref, ns) {
    ...
    if (ns.name !== "") {
        if (ns instanceof Service) {
            ...
            if (!ref && config.es6)
                ...
                push(escapeName(ref) + "." + escapeName(ns.name) + "WebSocket = (function() {");
            ++indent;
        }
        else {
            push("");
    ...
    if (ns instanceof Service) {
        if (ns.name !== "") {
            push("");
            push("return " + escapeName(ns.name) + "WebSocket;");
            --indent;
            push("})();");
        }
    } else {
...
function buildService(ref, service) {
    ...
    // Interface for the client to call...matches what's generated by grpc
    if (config.comments) {
        var typeDef = [
            "Properties of " + aOrAn(service.name) + "Client.",
            service.parent instanceof protobuf.Root ? "@exports " + escapeName("I" + service.name + "Client") : "@memberof " + exportName(service.parent),
            "@interface " + escapeName("I" + service.name + "Client")
        ];
        push("");
        pushComment(typeDef);
        service.methodsArray.forEach(function(method) {
            ...
            var comments = [
                ...
                "@instance",
                "@abstract",
                "@memberof " + exportName(service, true) + "Client"];

            if (!method.requestStream && !method.responseStream) {
                comments.push("@param {" + method.requestType + "} param");
                comments.push("@param {grpc.requestCallback<" + exportName(method.resolvedResponseType, true) + ">} callback");
            }
            else if (method.requestStream && !method.responseStream) {
                comments.push("@param {grpc.requestCallback<" + exportName(method.resolvedResponseType, true) + ">} callback");
                comments.push("@return {grpc.ClientWritableStream<" + method.requestType + ">}");
            }
            else if (!method.requestStream && method.responseStream) {
                comments.push("@param {" + method.requestType + "} param");
                comments.push("@return {grpc.ClientReadableStream<" + exportName(method.resolvedResponseType, true) + ">}");
            }
            else if (method.requestStream && method.responseStream) {
                comments.push("@return {grpc.ClientDuplexStream<" + method.requestType + "," + exportName(method.resolvedResponseType, true) + ">}");
            }
    ...
    push("}");
    push("");
    push("Reflect.setPrototypeOf(" + escapeName(service.name) + "WebSocket.prototype, wrapper.WSClient.prototype);");
    push("Reflect.setPrototypeOf(" + escapeName(service.name) + "WebSocket, wrapper.WSClient);");

    service.methodsArray.forEach(function(method) {
        method.resolve();
        var cbName = escapeName(method.name + "Callback");
        push("");
        pushComment([
            "Callback as used by {@link " + exportName(service) + "#" + escapeName(method.name) + "}.",
            // This is a more specialized version of protobuf.rpc.ServiceCallback
            "@memberof " + exportName(service) + "WebSocket",
            "@typedef " + cbName,
            "@type {function}",
        ...
        push("");
        pushComment(comments);
        push("Object.defineProperty(" + escapeName(service.name) + "WebSocket.prototype" + util.safeProp(method.name) + " = function " + escapeName(method.name) + "(" + params + ") {");
            ++indent;
            push(functionCall);
            --indent;
